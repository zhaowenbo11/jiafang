        ...
        <div className="space-y-6">
          ...
          <article className="rounded-[30px] border border-[#ead8cd] bg-white p-5 shadow-[0_16px_48px_rgba(94,48,33,0.06)]">
            <SectionTitle
              eyebrow="布料选择"
              title="当前推荐布料"
              description="先选 1 款主布，再根据客户反应切换升级布料。"
            />
            <div className="mt-5 space-y-3">
              {activeBusiness.fabrics.map((fabric, index) => (
                <SelectionCard
                  key={fabric.name}
                  title={fabric.name}
                  subtitle={activeBusiness.title}
                  detail={fabric.detail}
                  isActive={index === selectedFabricIndex}
                  accentSoft={activeBusiness.accentSoft}
                  variant="fabric"
                  swatch={fabric.swatch}
                  secondarySwatch={fabric.secondarySwatch}
                  detailSwatch={fabric.detailSwatch}
                  pattern={fabric.pattern}
                  onClick={() => setSelectedFabricIndex(index)}
                />
              ))}
            </div>
          </article>
